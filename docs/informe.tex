...
\subsection{Raw Validation (\texttt{src/validate.py})}

Validates the raw extracted data before any transformation:
\begin{itemize}[nosep]
    \item Verifies all required columns are present.
    \item Checks that \texttt{NumPersonas} contains no negative values.
    \item Validates regime codes are within \{S, C, E, I\}.
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item Maps departments to regions using \texttt{REGION\_MAP} (after normalization, ensuring correct mapping).
...
\midrule
Raw Validation & Validates raw data before transformation: required columns, no negative \texttt{NumPersonas}, valid regime codes (S/C/E/I)\\
...
\addlinespace
29 facilities from AMAZONAS/GUAVIARE dropped & \texttt{DEPT\_DANE\_CODES} missing these departments & Added \texttt{'AMAZONAS': '91'}, \texttt{'GUAVIARE': '95'} + new \texttt{validate\_department\_consistency()}\\
\addlinespace
Valle del Cauca (4.6M affiliates) mapped to ``Sin Region'' & \texttt{region} calculated before \texttt{DEPT\_NORMALIZE} corrected ``VALLE'' $\to$ ``VALLE DEL CAUCA'' & Moved \texttt{region} calculation to \texttt{clean\_affiliates()} after normalization; added \texttt{validate\_department\_region\_consistency()}\\
\bottomrule
\end{tabularx}
...
\textbf{Result:} Subsidized and Contributory regimes are nearly balanced nationally ($\sim$48\% vs $\sim$47.5\%). Special and Individual $<$5\%. The real disparity emerges at the regional level (see R5).
...
\textbf{Result:} Public facilities dominate high-complexity care (levels 3--4); private facilities concentrate in lower-complexity (levels 1--2). A marginal ``Mixta'' category (15 facilities) also exists.
...
\textbf{Result:} Smaller departments show highly variable ratios; some have excess capacity while others face critical shortages. SIN DEPARTAMENTO (non-geolocalized records) is excluded from this analysis.
...
\textbf{Result:} Andina region concentrates the majority. Pacifico shows a strong Subsidized majority (driven by Valle del Cauca, Choc\'{o}, and Nari\~{n}o). Amazonia has the highest proportional Subsidized concentration.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Care level gaps:} 61\% of facility records lack a care level. Missing values stored as NULL in the data warehouse.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Data quality:} Eleven critical data quality issues were identified and resolved, including silent data loss from special health districts, missing department codes, duplicate capacity rows, the ``NO APLICA'' gap, and the Valle del Cauca region mapping error.
